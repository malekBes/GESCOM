 ...
 * @author Mlek
 */
public class ReglementAvoirFournisseurFormDetail extends javax.swing.JInternalFrame {
     ...
     */
    public ReglementAvoirFournisseurFormDetail(String type_client, String type_reglement) {
         initComponents();
        setIconifiable(true);
        setMaximizable(true);
        setResizable(true);
...
//Remove rows one by one from the end of the table
    ...
    public void lstFacture() {
        ...
        if (id_four.isEmpty()) {
            ...
            Vector<Vector<Object>> data1 = FactureDao.afficherListAvoirFournisseur(id_four);
            df_reglement = new DefaultTableModel(data1, columnNames);

            for (int j = 0; j < df_reglement.getRowCount(); j++) {
                Object PU = df_reglement.getValueAt(j, 2);
                df_reglement.setValueAt(Config.Commen_Proc.formatDouble(Double.parseDouble(PU.toString())), j, 2);
            }

            JTable jTable = new JTable(df_reglement);
    ...
    private void initComponents() {
        ...
        setResizable(true);
        setTitle("Reglement Avoir Fournisseur");
    ...
    private void btn_validerActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_validerActionPerformed
        try {
            ...
            if (d != null) {
                ...
                String dt = daofacture.getDateAvoirFournisseur(txt_facture.getText());
                ...
                if ((date_reglement.compareTo(date_facture) > 0) || (date_reglement.compareTo(date_facture) == 0)) {
                    ...
                    if (x <= Double.valueOf(txt_m_total_facture.getText())) {
                        ...
                        if (fdao.ajouterReglementAvoirFournisseur(d)) {

                            JOptionPane.showMessageDialog(null, "Reglemnet à été bien enregistré !");
                            update_table();
                            if (Double.parseDouble(txt_m_restant.getText()) == 0.0) {
                                daofacture.setAvoirReglementFournisseur(txt_facture.getText(), "Oui");
                                clearForm("all");
                            } else {
                                daofacture.setAvoirReglementFournisseur(txt_facture.getText(), "Semi");
                                clearForm("semi");
                            }
    ...
    private void ComboBox_fourActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ComboBox_fourActionPerformed
        ...
        if (b) {
            if (ComboBox_four.getSelectedItem() != "-") {
                ...
                df_avoir = r.afficherTableFactureFournisseur(Table_avoir, ss);
                Double tot = 0.0;
