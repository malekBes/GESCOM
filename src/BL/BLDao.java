...
//sql = "insert into client_info(id,name,address,contact,datee) values (NULL,'" + c.Adresse + "','" + c.Compte_Bank + "','" + c.Compte_Bank + "','" + c.Adresse + "')";
    ...
    public void afficherHistPrixBL(JTable table, String NomClient, String Ref_article) {
        ...
        try {
            ...
            sql = "select ld.ref_article, ld.designation_article, ld.`prix_u`, ld.qte, ld.remise, ld.tva,DATE_FORMAT(d.date_bl,'%d/%m/%Y') "
                    ...
                    + " and d.id_client = c.numero_client and c.nom='" + NomClient + "' "
                    + " and ld.ref_article = '" + Ref_article + "' order by d.`date_bl` desc limit 1";
            System.out.println(sql);
    ...
    public DefaultTableModel afficherDetailDevis(JTable table, String Num_Devis) {
        ...
        try {
            String sql;
            //numero_Client, nom, num_Tel, adresse, Ville, pays, code_Postale, zone_Geo, id_Fiscale, Email, site, fax, adresse_livraison, contact_Client, type_Client, Etat_Paiement, agence, Compte_Bank, Fournisseur_Preced, actif, Id_Commercial
            sql = "SELECT id,ref_article,designation_article,prix_u,qte,total_ht,remise,tva,total_ttc from ligne_devis where id_devis='" + Num_Devis + "' group by ref_article";
    ...
    public DefaultTableModel afficherDetailRechercheDevis(JTable table, String Num_Devis) {
        ...
        try {
            String sql;
            //numero_Client, nom, num_Tel, adresse, Ville, pays, code_Postale, zone_Geo, id_Fiscale, Email, site, fax, adresse_livraison, contact_Client, type_Client, Etat_Paiement, agence, Compte_Bank, Fournisseur_Preced, actif, Id_Commercial
            sql = "SELECT * from ligne_devis where id_devis='" + Num_Devis + "' group by ref_article";
    ...
    public String afficherFacturePourAnnuler(JTable table, String Num_Devis) {
        ...
        try {
            String sql;
            //numero_Client, nom, num_Tel, adresse, Ville, pays, code_Postale, zone_Geo, id_Fiscale, Email, site, fax, adresse_livraison, contact_Client, type_Client, Etat_Paiement, agence, Compte_Bank, Fournisseur_Preced, actif, Id_Commercial
            sql = "SELECT distinct DATE_FORMAT(d.date_facture,'%d/%m/%Y'),d.ttc,c.nom,d.type_facture,d.passager from facture d left join client c  on c.numero_Client=d.id_client where  d.num_facture='" + Num_Devis + "' and d.statut =1 and d.reglement = 'Non' ";
    ...
    public DefaultTableModel afficherPreCommandeAchatPourModif(JTable table, String Num_Devis) {
        ...
        try {
            String sql;
            //numero_Client, nom, num_Tel, adresse, Ville, pays, code_Postale, zone_Geo, id_Fiscale, Email, site, fax, adresse_livraison, contact_Client, type_Client, Etat_Paiement, agence, Compte_Bank, Fournisseur_Preced, actif, Id_Commercial
            sql = "SELECT `id`, `ref_article`, `designation_article`, `qte` from ligne_pre_commande where id_pre_commande='" + Num_Devis + "' ";
    ...
    public DefaultTableModel afficherDetailAvoir(JTable table, String Num_Devis) {
        ...
        try {
            String sql;
            //numero_Client, nom, num_Tel, adresse, Ville, pays, code_Postale, zone_Geo, id_Fiscale, Email, site, fax, adresse_livraison, contact_Client, type_Client, Etat_Paiement, agence, Compte_Bank, Fournisseur_Preced, actif, Id_Commercial
            sql = "SELECT c.nom,la.* from ligne_avoir la left join avoir a on a.num_avoir=la.id_avoir left join client c on c.numero_client=a.id_client  where id_avoir='" + Num_Devis + "'  group by ref_article ";
...
//            sql = "SELECT DISTINCT lf.num_facture, b.`Num_bl`, DATE_FORMAT(b.`date_bl`,'%d/%m/%Y'), b.`remise`, f.`TTC` from ligne_facture lf, bl b, facture f where lf.num_bl=b.num_bl and f.num_facture=lf.num_facture and lf.num_facture='" + Num_Devis + "' and b.statut =1";
           // sql = "SELECT DISTINCT lb.id, lb.id_bl, lb.ref_article, lb.designation_article, lb.qte, lb.prix_u, lb.total_ht,lb.total_ttc, lb.tva,lb.remise,f.num_facture, if(c.adresse is null , '', c.adresse) as 'c.adresse',if(c.nom is null,f.passager,c.nom) as 'c.nom',if(c.id_fiscale is null , '',c.id_fiscale) as 'c.id_fiscale',  if(date_bl is null ,'', DATE_FORMAT(`date_bl`,'%d/%m/%Y') ) as date_bl_fomrated FROM `ligne_bl` lb left join ligne_facture lf on lb.id_bl=lf.num_bl left join facture f on f.num_facture=lf.num_facture  left join client c on c.numero_client=f.id_client left join bl bl on bl.num_bl=lb.id_bl   WHERE f.num_facture = '" + Num_Devis + "'   order by lb.id_bl";
            sql = "SELECT DISTINCT lb.id, lb.id_bl, lb.ref_article, lb.designation_article, lb.qte, lb.prix_u, lb.total_ht,lb.total_ttc, lb.tva,lb.remise,f.num_facture, if(c.adresse is null , '', c.adresse) as 'c.adresse',if(c.nom is null,f.passager,c.nom) as 'c.nom', if(c.id_fiscale is null , f.code_tva_passager ,c.id_fiscale) as 'c.id_fiscale',  if(date_bl is null ,'', DATE_FORMAT(`date_bl`,'%d/%m/%Y') ) as date_bl_fomrated "
                    + " FROM `ligne_bl` lb left join ligne_facture lf on lb.id_bl=lf.num_bl left join facture f on f.num_facture=lf.num_facture  left join client c on c.numero_client=f.id_client left join bl bl on bl.num_bl=lb.id_bl   WHERE f.num_facture = '" + Num_Devis + "'   ";
    ...
    public Vector<Vector<Object>> afficherDetailFactureModif(JTable table, String Num_Devis) {
        ...
        try {
            String sql;
            //numero_Client, nom, num_Tel, adresse, Ville, pays, code_Postale, zone_Geo, id_Fiscale, Email, site, fax, adresse_livraison, contact_Client, type_Client, Etat_Paiement, agence, Compte_Bank, Fournisseur_Preced, actif, Id_Commercial
            sql = "SELECT distinct c.nom, b.`Num_bl`, DATE_FORMAT(b.`date_bl`,'%d/%m/%Y'),b.total_HT, b.`remise`,b.montant_tva, b.`Total_TTC` from ligne_facture lf, bl b, client c where c.numero_client=b.id_client and lf.num_bl=b.num_bl and lf.num_facture='" + Num_Devis + "' and statut =1";
    ...
    public void afficherDetailFacturePassager(JTable table, String Num_Devis) {
        ...
        try {
            String sql;
            //numero_Client, nom, num_Tel, adresse, Ville, pays, code_Postale, zone_Geo, id_Fiscale, Email, site, fax, adresse_livraison, contact_Client, type_Client, Etat_Paiement, agence, Compte_Bank, Fournisseur_Preced, actif, Id_Commercial
            sql = "SELECT * from ligne_facture_passager lf where lf.id_facture_passager='" + Num_Devis + "'";
