...
public abstract class ConvertChiffreToString {
	private static String resultfinal, result,s, frac, tranche, dec, cent, un, dix;
	private static int rang,rang1, virgule, lg;
	private static String traiter(String nombre) {
		...
		frac= "";
		if (virgule> -1) {
			frac= nombre.substring(virgule+ 1, ((nombre.length()- virgule)< 4? nombre.length(): virgule+ 4));
        ...
        private static String traiter1(String nombre) {
		nombre= nombre.replace(',', '.');
		nombre= nombre.replace(" ", "");
		if (nombre.charAt(0)== '-')
			nombre= nombre.substring(1);
		...
		if ((nombre.equals("#Erreur"))||(nombre.equals("#Trop grand")))
			...
			while (rang> 0)	{
				...
				if (result== "") {
					...
					tranche= nombre.substring(nombre.length()- (rang1* 3), nombre.length()- ((rang1- 1)* 3));
				dec= tranche;
				dec= dec.substring(1, 3);
				if (dec.length()== 1) {
					un= dec;
				...
				switch (Integer.parseInt(dec)) {
					case 0:
						if (!cent.equals("0"))
							result+= n1000[rang1- 1];
					break;
					case 1: case 2: case 3: case 4: case 5: case 6: case 7: case 8: case 9: case 10: case 11: case 12: case 13: case 14: case 15: case 16: case 17: case 18: case 19:
						result+= n1[Integer.parseInt(dec)]+ n1000[rang1- 1];
					break;
					case 71: case 72: case 73: case 74: case 75: case 76: case 77: case 78: case 79:
						result+= n70[Integer.parseInt(un)- 1]+ n1000[rang1- 1];
					break;
					case 91: case 92: case 93: case 94: case 95: case 96: case 97: case 98: case 99:
						result+= n90[Integer.parseInt(un)- 1]+ n1000[rang1- 1];
					break;
					default:
						if ((!dix.equals("0"))&&(un.equals("1")))
							result+= n10[Integer.parseInt(dix)]+ " et Un"+ n1000[rang1- 1];
						else
							result+= n10[Integer.parseInt(dix)]+ n1[Integer.parseInt(un)]+ n1000[rang1- 1];
				}
				rang1--;
			}
                        // resultfinal avant virugle
                        // result aprés virule 
                        
			/*if (resultfinal.equals(""))
				resultfinal = " Zero "+ devise+ " et" + result +" " + sou;
			else
				resultfinal+= " "+ devise+ " et" + result +" "+ sou;
                       */
                        if(result.equals("")){
                            s = resultfinal + " "+ devise + " et Zero "+ sou ;
                        }else if(resultfinal.equals("")){
                            s = " Zero "+ devise+ " et" + result +" " + sou;
                        } else{
                            s= resultfinal+ " "+ devise+ " et" + result +" "+ sou;
                        }
	...
	public static String FR(String nombre) {
		return FR(nombre, "Dinars", "Millimes");
	}
}
